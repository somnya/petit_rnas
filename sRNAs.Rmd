...
baits_flowers = baits_flowers %>% mutate(cluster = rownames(baits_flowers)) %>% group_by(cluster) %>% summarise(female_exp = mean(c(F1B_dicer, F2B_dicer, F3B_dicer)), 
                                                                                                      ...
                                                                                                      type = "Sex-biased in flowers") %>% add_count(type)
...
baits_leaves = baits_leaves %>% mutate(cluster = rownames(baits_leaves)) %>% group_by(cluster) %>% summarise(female_exp = mean(c(F1L_dicer, F2L_dicer, F3L_dicer)), 
                                                                                                   ...
                                                                                                   male_biased = male_exp>female_exp,
                                                                                                   type = "Sex-biased in leaves") %>% add_count(type)
...
ggplot(bait_df) +
  ...
  geom_jitter(aes(female_exp, male_exp, fill=male_biased, shape=MIRNA, color=MIRNA), size=6) +
...
```{r}
# check if Y is mapping in females
# Get clusters from Y 
rownames(anno_ptgs) = anno_ptgs$Name
y_clusters = anno_ptgs[grep("scaffold", anno_ptgs$Chrom), ]
#y_clusters = as.data.frame(cpm(drop_na(matrix_ptgs[rownames(y_clusters), ]), log = T)) # logRPM
y_clusters = as.data.frame(matrix_ptgs[rownames(y_clusters), ]) # Raw reads
y_clusters = y_clusters  %>% select(grep('F', colnames(y_clusters))) %>% mutate(cluster = rownames(y_clusters))
y_clusters = melt(y_clusters, id.vars = "cluster", variable.name = "sample")


ggplot(y_clusters) +
  geom_point(aes(x=as.factor(cluster), y=value, color=sample)) +
  scale_color_manual(values = met.brewer("Lakota")) +
  labs(x="Y chromosome sRNA clusters", y="Aligned reads") +
