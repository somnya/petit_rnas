...
flo_sbg_fe_d = surco_gene(flo_sbg_fe_d, 19022354, 10510431, 8511923)
colnames(flo_sbg_fe_d) = c("gene", "female_exp")
# Males
flo_sbg_ma_d = fread("results/gene-level/flowers_depth_males.tsv")
flo_sbg_ma_d = surco_gene(flo_sbg_ma_d, 6486661, 9227331, 9622793)
colnames(flo_sbg_ma_d) = c("gene", "male_exp")

flo_depth = full_join(flo_sbg_fe_d, flo_sbg_ma_d, by = "gene")

flo_depth$female_exp[is.na(flo_depth$female_exp)] = -5 # replace NA for "0" mapping
flo_depth$male_exp[is.na(flo_depth$male_exp)] = -5
...
ggplot(flo_depth) +
  annotate(geom = "polygon", x = c(-Inf, Inf, Inf), y = c(-Inf, Inf, -Inf), fill = "gray8", alpha = 0.2 ) +
  geom_abline(intercept = 0, slope = 1, alpha=0.6, color="red2", size=1, linetype="dashed") +
  geom_jitter(aes(female_exp, male_exp), color=met.brewer("Derain", 1), size=4) +
...
rm(baits_flowers, baits_leaves, bait_df)
```
...
modules = cluster_leiden(my_network, resolution_parameter = 0.001, objective_function = "modularity") # 1 cluster in flowers+leaves vs 28 clusters only in flowers
# re-anotate network
network_annotation = bait_df %>% select(gene, male_biased) %>% distinct(gene, .keep_all = TRUE) %>% filter(gene %in% modules$names) %>% mutate(bait = gene == "Silat_chr12_Gene.1440")
...
ggraph(my_network, layout = "kk", circular = F) +
  ...
  geom_node_point(aes(fill = male_biased, color=bait, shape=bait), alpha = 0.8, size = 4) +
  scale_fill_manual(values = met.brewer("Derain")) +
  scale_shape_manual(values = c(21, 19)) +
...
rm(de_genes, weight_mat, link_list, my_network, network_annotation)
```
