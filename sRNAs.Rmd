...
## 2.- Quantification analysis and differential expression
...
comparisons = list(list("Sex-biased in flower buds", "Tissue-biased in males"),list("Sex-biased in leaves", "Tissue-biased in females"))
...
write.table(female[["Lima-Voom table"]] %>% mutate(Name = rownames(female[["Lima-Voom table"]])), file = "results/differential_expression/stissue_biased_females_ptgs_lvoom.tsv", quote = F, sep = "\t", col.names = T, row.names = F)
...
write.table(male[["Lima-Voom table"]] %>% mutate(Name = rownames(male[["Lima-Voom table"]])), file = "results/differential_expression/stissue_biased_males_ptgs_lvoom.tsv", quote = F, sep = "\t", col.names = T, row.names = F)
```
...
summary(keep) # 6459 will be removed 
kall = kall[keep,]
rm(keep)
...
fwrite(as.data.table(flowers_genes[["clusters"]]), "try1.txt")
```
...
sRNA_clusters = as.data.table(anno_ptgs) # transform
...
genesCoordinates = fread("annotation/ALLGenesFromVulgarisIntoLatifolia.txt")
genesCoordinates[, ':='(query_sequence = NULL, flag = NULL, mapping_quality=NULL)] # remove columns
genesCoordinates[, gene_name := strsplit(gene_name, '::')[[1]][1], by=1:nrow(genesCoordinates)] # keep simple gene IDs
genesCoordinates[, min := min(start_pos, end_pos), by=1:nrow(genesCoordinates)]
genesCoordinates[, max := max(start_pos, end_pos), by=1:nrow(genesCoordinates)]
# Add 400bp upstream and downstream
genesCoordinates[, min := min - 400, by=1:nrow(genesCoordinates)]
genesCoordinates[, max := max + 400, by=1:nrow(genesCoordinates)]

# select for DE
sRNA_clusters = sRNA_clusters[Name %in% flowers[["clusters"]]] # Select DE sNRAs
genesCoordinates = genesCoordinates[gene_name %in% flowers_genes[["clusters"]]] # select DE genes

# Initiate data.table containing the header for two columns: gene name and name of overlapping clusters
header = list('gene_name', 'overlapping_cluster')
fwrite(header, 'Overlapping_cluster_numbers.txt', append = FALSE, sep='\t')

# For each gene, get all overlapping siRNA clusters
for (i in seq(1, nrow(genesCoordinates))) {
  # Current gene info
  CurrentGene <- genesCoordinates$gene_name[i]
  CurrentChr <- genesCoordinates$scaffold[i]
  Currentmin <- genesCoordinates$min[i]
  Currentmax <- genesCoordinates$max[i]
  ...
  siRNAsubset <- sRNA_clusters[(Chrom == CurrentChr)&(min < Currentmax)&(max > Currentmin)]
  # write a line with output and append it to output data.table
#  line <- list(CurrentGene, nrow(siRNAsubset))
  line = as.data.frame(siRNAsubset$Name) %>% mutate(CurrentGene) %>% relocate(CurrentGene)
  fwrite(line, 'Overlapping_cluster_numbers.txt', append = TRUE, sep='\t')
}
